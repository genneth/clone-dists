function p = prob_zero_basal(r, gamma, ts)
...
parfor i = 1:N
    z = exp(2i*pi/N * (i-1));
    next_samples(i,:) = generating_function2(r, gamma, ts, z, z);
end
next_ps = abs(fft(next_samples, [], 1)) / N;

    function e = rel_err
        e = max(abs(curr_ps(1,:) - next_ps(1,:)) ./ next_ps(1,:));
    end

    function e = abs_err
        e = max(abs(curr_ps(1,:) - next_ps(1,:)));
    end

while rel_err > 1e-4 && abs_err > 1e-7
