function pb = clone_dist(r1, r2, ts, N0)
...
for j = 1:N
    z = exp(2i*pi/N * (j-1));
    next_samples(j,:) = generating_function(r1, r2, ts, z, z);
end
...
while rel_err > 1e-3 && abs_err > 1e-5
    curr_samples = next_samples;
    curr_ps      = next_ps;
    next_s2      = zeros(N, numel(ts));
    for j = 1:N
        z = exp(2i*pi/(2*N) * (2*j-1));
        next_s2(j,:) = generating_function(r1, r2, ts, z, z);
    end
    % interleave
    N = N*2;
    next_samples = zeros(N, numel(ts));
