...
ones = cellfun(@(d)(d(1,2) / sum(d(:,2))), data);
twos = cellfun(@(d)(d(2,2) / sum(d(:,2))), data);

theory_ts = logspace(ts(1), ts(end), 40);
theory_ones = zeros(size(theory_ts));
theory_twos = zeros(size(theory_ts));
parfor i=[1:numel(theory_ts)]
    t = theory_ts(i);
    predictions = exact_pops(0.7676, 0.20, 0.7676, t, 32);
    theory_ones(i) = predictions(1+1);
    theory_twos(i) = predictions(2+1);
end

semilogx(ts, ones, '+b', ts, twos, 'xr', ...
    theory_ts, theory_ones, '-b', theory_ts, theory_twos, '-r');
